...
export function transposeUp(chordItem: ChordItem): ChordItem {
  if (
  ...
  if (
    chordItem.simpleChord?.rootNote?.indexOf("#") > 0 ||
    chordItem.simpleChord?.rootNote?.indexOf("♯") > 0 ||
    chordItem.simpleChord?.rootNote === "E" ||
    chordItem.simpleChord?.rootNote === "B"
  ) {
  ...
  if ((chordItem?.simpleChord?.bassNote?.length || 0) > 0) {
    if (
      ...
      (chordItem?.simpleChord?.bassNote?.indexOf("♯") || -1) > 0 ||
      chordItem?.simpleChord?.bassNote === "E" ||
      chordItem?.simpleChord?.bassNote === "B"
    ) {
